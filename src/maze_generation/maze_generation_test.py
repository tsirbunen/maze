...
def test_maze_generation_variables_are_correctly_initialized_for_mazes_of_different_size():
    ...
    for size in sizes:
        ...
        (unlabeled_nodes, labeling_statuses, node_labels, node_groups, connections) = initializer.get_result()
...
def test_neighbor_cells_are_correctly_identified():
    ...
    for (size, node, expected_neighbors) in test_input_expected_output_data:
        initializer = VariablesInitializer(size)
        twister = Twister(MazeParameters(size, MazeType.SINGLE), dispatch_event, initializer.get_result())
...
def test_twisting_produces_proper_variable_values():
    ...
    for size in test_input_data:
        ...
        assert_node_labels_have_been_assigned_max_allowed_values(node_groups, node_labels)
...
def test_twisting_publishes_an_event_for_each_node_labeling_action(mocker):
    ...
    for i in range(len(test_input_data)):
        ...
        initializer = VariablesInitializer(size)
        twister = Twister(MazeParameters(size, MazeType.SINGLE), TestPublisher.dispatch_event, initializer.get_result())
        twister.perform()
...
def assert_node_labels_match_node_group_data(node_groups, node_labels):
    for label in range(0, len(node_groups)):
        label_indexes_in_node_labels = [i for i in range(len(node_labels)) if node_labels[i] == label]
...
def test_merging_produces_a_valid_maze():
    ...
    merger = Merger(MazeParameters(size, MazeType.SINGLE), dispatch_event, twisted_variables)
    merger.perform()
    ...
    assert_generated_maze_contains_no_loops_and_all_nodes_are_connected(size, connections)
...
def assert_generated_maze_contains_no_loops_and_all_nodes_are_connected(size, connections):
    visited_statuses = []
    for i in range(0, size * size):
...
def visit_all_children_only_once(node, parent, visited_statuses, connections):
    ...
    for child in children:
        if child != parent:
            visit_once = visit_all_children_only_once(child, node, visited_statuses, connections)
            if not visit_once:
                return False
    return True
